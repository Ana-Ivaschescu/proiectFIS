...
public class RequestTest extends ApplicationTest {
    ...
    public  void Setup()
    {
        ...
        items.add(p.getName() + "  |  " + p.getPlaying_position() + "  |  " + p.isAvailable());
        controller_tm.player_list.setItems(items);
        controller_tm.player_list.getSelectionModel().select(0);
        controller_tm.requestSentLabel = new Label();
        controller_tm.setPa(DataManager.searchPA(TestDataHolder.TEST_PA_NAME, pa_hash_list));
        controller_tm.setTm(DataManager.searchTM(TestDataHolder.TEST_TM_NAME, tm_hash_list));
        controller_tm.setPa_hash_list(pa_hash_list);
        controller_tm.setTm_hash_list(tm_hash_list);
    }
